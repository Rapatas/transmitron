...
// NOLINTNEXTLINE
class Layout : public wxCommandEvent
{
public:
  ...
  [[nodiscard]] wxEvent *Clone() const override { return new Layout(*this); }

  void setPerspective(std::string perspective) {
    mPerspective = std::move(perspective);
  }

  [[nodiscard]] std::string getPerspective() const { return mPerspective; }

  [[nodiscard]] wxDataViewItem getItem() const { return mItem; }

  void setItem(wxDataViewItem item) { mItem = item; }
