...
Messages::Messages(const ArtProvider &artProvider) :
  FsTree("Messages", static_cast<size_t>(Column::Max), artProvider),
...
void Messages::leafValue(Id id, wxDataViewIconText &value, unsigned int col)
  const {
  (void)col;
...
std::unique_ptr<FsTree::Leaf> Messages::leafLoad(
  Id id,
  ...
  if (!messageFile.is_open()) { return nullptr; }
  ...
  if (!sbuffer.empty()) {
    if (!nlohmann::json::accept(sbuffer)) { return nullptr; }
...
Messages::Message::Message(const MQTT::Message &msg) :
  MQTT::Message(msg) //
{}
