...
size_t Subscription::attachObserver(Observer *observer) {
  ...
  if (mState == State::Subscribed) { onSubscribed(); }
...
void Subscription::onSubscribed() {
  for (const auto &[id, observer] : mObservers) { observer->onSubscribed(); }
}

void Subscription::onUnsubscribed() {
  for (const auto &[id, observer] : mObservers) { observer->onUnsubscribed(); }
}

void Subscription::onMessage(const mqtt::const_message_ptr &msg) {
...
QoS Subscription::getQos() const { return mQos; }
